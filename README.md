...
standard output without any additional informational messages.

For example, here's a CLI pipeline to classify all code changes in a `git`
repository from the last 90 days, and find the top 10 files that were most
frequently changed in order to fix bugs (saving intermediate results for
further processing in `_git-changes.tsv`):

```sh
git log --format=%H --since="90 days ago" \
  | while read hash
    do
        cls=$(ai-cat.py -q -o <<PROMPT | grep -o '^\**class: *[a-z]*' | sed 's/^\**class: *//'
# === System ===

Respond as an experienced software developer.

Read the git commit message below carefully.

Briefly explain your impression regarding whether the associated patch is
likely to

 * implement a new feature,
 * fix a bug,
 * or do general maintenance work.

When in doubt, pick maintenance.

In the last line of your response, give a final classification verdict by
writing exactly one of the following 3 lines without any changes or formatting:
