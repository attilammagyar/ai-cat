...
" Asynchronous version of the system() function.
"
" Parameters:
" - command (string):      the shell command to run
" - stdin (string):        string to pass to the command on its standard input
" - ok_callback (string):  name of a function to be called sooner or later
"                          after the command stops with a 0 exit status;
" - err_callback (string): name of a function to be called sooner or later
"                          after the command stops with a non-zero exit status
" - data (any):            custom data to pass to the callbacks
"
" Signature of the callbacks: func(file_name, stdout, stderr, data), where:
" - file_name (string):    the name of the file in the buffer where the
"                          SystemAsync() call was initiated
" - stdout (list):         contents from the standard output of the command
" - stderr (list):         contents from the standard error of the command
" - data (any):            the custom data that was passed to
"                          SystemAsync()
function! SystemAsync(command, stdin, ok_callback, err_callback, data)
    ...
    if has("win32")
        ...
        if v:shell_error == 0
            call SystemAsyncCallFuncByName(a:ok_callback, l:current_file, l:output, [""], a:data)
        else
            call SystemAsyncCallFuncByName(a:err_callback, l:current_file, [""], l:output, a:data)
        endif
...
function! SystemAsyncCallFuncByName(name, arg1, arg2, arg3, arg4)
...
function! SystemAsyncCheckResults()
    ...
    for l:task_id in l:ok_calls
        ...
        call SystemAsyncCallFuncByName(l:task[2], l:task[0], l:stdout, l:stderr, l:task[4])
    endfor
    ...
    for l:task_id in l:fail_calls
        ...
        call delete(l:task[1] . ".out")
        call delete(l:task[1] . ".err")
        call SystemAsyncCallFuncByName(l:task[3], l:task[0], l:stdout, l:stderr, l:task[4])
    endfor
...
function! AICatTabReplace() range
    ...
    let l:ai_cat_cmd = "ai-cat.py replace " . shellescape(expand("%"))
